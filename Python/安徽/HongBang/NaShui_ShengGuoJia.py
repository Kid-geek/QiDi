...
def get_url(page_num):
    ...
    for num in range(1, int(page_num) + 1):
        url = 'http://www.creditah.gov.cn/remote/483/index_'+str(num)+'.htm'
...
def get_info(url):
    ...
    info_dict['评定所属期'] = td_soup.find_all('td')[5].text.replace(' ', '').replace('\r','').replace('\n','').replace('\t','')
    info_dict['主管税务机关'] = td_soup.find_all('td')[7].text.replace(' ', '').replace('\r','').replace('\n','').replace('\t','')
    info_dict['统一社会信用代码'] = td_soup.find_all('td')[9].text.replace(' ', '').replace('\r','').replace('\n','').replace('\t','')
    info_dict['纳税信用等级'] = td_soup.find_all('td')[11].text.replace(' ', '').replace('\r','').replace('\n','').replace('\t','')
